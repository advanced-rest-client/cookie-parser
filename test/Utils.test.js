...
describe('Utils', () => {
  ...
  describe('matchesDomain()', () => {
    it('returns false when no url', () => {
      assert.isFalse(Utils.matchesDomain('.api.com', undefined));
    });

    it('returns false when no argument', () => {
      const uri = new URL(baseUrl);
      assert.isFalse(Utils.matchesDomain('', uri));
    });

    it('returns true when domain are the same', () => {
      const uri = new URL(baseUrl);
      assert.isTrue(Utils.matchesDomain('bar.com', uri));
    });
    ...
    it('returns false when dot in argument and url is deep subdomain', () => {
      ...
      assert.isFalse(Utils.matchesDomain('.bar.com', uri));
    });
  });

  describe('matchesPath()', () => {
    it('returns false when no url', () => {
      assert.isFalse(Utils.matchesPath('/', undefined, undefined));
    });

    it('returns true when no argument', () => {
      const uri = new URL(baseUrl);
      assert.isTrue(Utils.matchesPath('', uri, baseUrl));
    });
    ...
    it('returns true when URL has single separator', () => {
      ...
      assert.isTrue(Utils.matchesPath('/', uri, url));
    });
  ...
  describe('getPath()', () => {
    ...
    it('returns path value', () => {
      const result = Utils.getPath('https://api.com/api/test/ignore');
      assert.equal(result, '/api/test');
    });

    it('ignores query string', () => {
      const result = Utils.getPath('https://api.com/api/?a=b');
      assert.equal(result, '/api');
    });

    it('ignores hash part of the url', () => {
      const result = Utils.getPath('https://api.com/api/#access_token=...');
      assert.equal(result, '/api');
    });
  });
});
